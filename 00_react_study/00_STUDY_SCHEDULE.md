...
| 52  | 06/02 ✅ | useOptimistic       | 좋아요 버튼 낙관적 업데이트 구현           | 서버 응답 전 즉시 하트 UI 반영         |
| 53  | 06/05 ✅ | useOptimistic 심화  | 실패 시 롤백 처리                          | 네트워크 오류 → 원래 값 복원           |
| 54  | 06/11 ✅ | use()               | Supabase Promise를 `use()`로 처리          | Suspense와 함께 데이터 로딩 선언적으로 |
| 55  | 06/11 ✅ | Server Actions      | 팔로우/언팔로우를 Server Action으로 구현   | `'use server'` 함수 + `revalidatePath` |
| 56  | 06/11 ✅ | Phase 5 복습        | React 19 기능 적용 전/후 코드 diff 비교    | 어떤 코드가 얼마나 단순해졌는지 정리   |
...
| 68  | 06/29 ✅ | 전체 복습 & 회고         | Phase 1-6 개념 마인드맵 + 개선 내역 정리   | 학습 일지 완성                         |
