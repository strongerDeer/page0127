...
> **업데이트(2026-07-23): 무료 계정 사정으로 1차 범위를 축소.**
> 클라우드 개발 프로젝트 대신 **로컬 Supabase(Docker)** 로 "로컬 개발만" 운영과 분리한다.
> CI·Vercel Preview는 당분간 운영을 그대로 쓰고(스모크는 읽기·미인증이라 위험 낮음),
> 완전 분리(클라우드 개발 프로젝트 or Pro Branching)는 다음 라운드로 미룬다.
> 실행 계획은 이 축소안 기준으로 재작성됨 → `docs/superpowers/plans/2026-07-23-page0127-dev-prod-separation.md`
