...
**포화 케이스의 오차 방향(허용 쪽):** 이 방향은 안전하다 — 사용자를 막지 않고,
재분석 남용은 이미 `checkUsageLimit`의 월간 한도로 막혀 있다. 또한 타깃 사용자
(월 1~4권)가 100권에 도달하려면 수년이 걸린다.

**삭제·별점 제거의 오차 방향(막는 쪽 — 정정):** 오차 방향이 항상 허용 쪽이라고
적었던 것은 틀렸다. 삭제·별점 제거는 **현재 총량만 줄이고 저장된
`analyzed_books_count`는 그대로 두므로**, 델타가 실제로 새로 읽은 권수보다 작아진다.
`Math.max(0, current − analyzed_at_time)`이므로 분석 이후 책 2권을 삭제(또는 별점
제거)하고 5권을 새로 읽으면 델타가 `5 − 2 = 3`이 되어, `PublicLibraryHeader`의
`< 5` 게이트에 **여전히 막힌다** ("이전 분석 이후 새로 읽은 책이 3권이에요").
옛 `completed_date >` 로직에서는 통과했을 상황이다.

이 오차는 **수용한다.** 다음 분석을 한 번 실행하면 `analyzed_books_count`가 현재
총량으로 다시 맞춰져 스스로 회복되고, 영향 범위도 "분석 이후 책을 지운 사용자가
다음 재분석까지 삭제한 만큼 더 읽어야 한다"로 좁다. 정확히 고치려면 실제 총량을
