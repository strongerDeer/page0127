...
- 작성일: 2026-07-29
- 범위: 책 표지 셀을 `shared/ui` 컴포넌트로 추출하고 Figma 에 올린다
...
책 표지의 도메인 셰이프(비대칭 radius + 책등 그라데이션)가 **10개 파일에는 손으로 붙어 있고 6개 파일에는 없다.** 이를 `BookCover` 컴포넌트로 추출해 13개 파일을 흡수하고, 대체 조판을 하나로 통일한다.
...
`BookGridItem` · `ReadingNowStrip` · `BookDetailContent`
...
grep -rn "표지 없음"  --include="*.tsx" src app | wc -l                                        # 0
```
...
`표지 없음` 은 컴포넌트가 제목을 쓰므로 어디에도 남지 않는다 — 여기는 0이 맞다.
...
1. `BookCover` 가 `shared/ui` 에 있고 13개 파일이 그것을 쓴다
