...
### 8.1 이사 시점의 검증 (완료, 이후 교체됨)

이사 직후에는 **이사 전 `globals.css` 46개를 스냅샷(`baseline.json`)으로 얼려두고 값과 이름을 대조**했다. `--secondary`·`--muted` 2개만 예외로 두었다(§5.2). 이 방식이 이사 자체는 정확히 검증했다 — 44개 값 동일, 2개 의도된 변경.

> **교체 (2026-07-27):** 스냅샷 방식은 *이사*를 검증하려고 만든 것이라, **값이 정당하게 바뀌는 순간부터 방해가 된다.** 다크모드(라운드 4)나 브랜드 색 조정이 오면 테스트가 실패하는데 그것이 버그인지 의도인지 구분할 방법이 없고, 갱신 경로도 애매했다. 이사가 끝나 역할을 다했으므로 아래 8.2 구조로 교체하고 `baseline.json`·`update-baseline` 스크립트를 삭제했다.

### 8.2 상시 회귀 방어 — 깨지는 방식마다 다른 장치

토큰이 깨지는 방식이 셋이라 각각 다른 장치가 맡는다.

| 무엇이 깨지나 | 무엇이 막나 | 왜 그 장치인가 |
|---|---|---|
| **값이 실수로 바뀜** | 커밋된 `dist/tokens.css` 의 **git diff** | 생성물을 커밋하므로(§7.3) 값 변경이 리뷰에 그대로 드러난다. 테스트보다 정확하고, 정당한 변경을 막지도 않는다 |
| **이름이 사라짐** | `apps/page0127/app/token-usage.test.ts` | 소스의 `var(--…)` 를 전부 모아 정의 존재를 확인. §4.2가 지키려던 것을 **소비처 기준**으로 겨냥한다 |
| **구조가 무너짐** | `packages/design-tokens/tests/tokens.test.ts` | semantic 이 원색을 `var()` 로 참조하지 않고 값을 복사하면 2층 구조가 무의미해진다 |
| **배선이 끊김** | `apps/page0127/app/globals.test.ts` | 토큰이 살아 있어도 `@theme inline` 매핑이 빠지면 `bg-rank-up` 같은 클래스가 조용히 죽는다 |

**이름 검사를 소비처 기준으로 바꾼 것이 핵심이다.** 이전에는 "생성물이 옛 목록과 같은가"를 봤는데, 이제는 "**쓰는데 정의가 없는 변수가 있는가**"를 본다. 값이 바뀌어도 깨지지 않고, 이름이 사라지면 반드시 깨진다.

조사 결과 소스가 쓰는 CSS 변수 49종 중 외부 라이브러리 것(`--radix-*` 2개)과 CSS Module 지역 변수(`--size`)를 빼면 **정확히 semantic 46개** — 죽은 토큰이 하나도 없다.

네 테스트 모두 **일부러 깨뜨려 실패하는 것을 확인**했다(토큰 이름 제거 → 소비처 대조 실패, 값 복사로 변조 → 구조 테스트 실패).
...
한동안 이것이 라운드 2·3을 제약할 것으로 봤다. 폰트가 Pretendard로 바뀐 뒤로는 **에이전트가 그 텍스트 노드의 `characters`조차 수정할 수 없었기** 때문이다 — Plugin API는 텍스트 변경 전 현재 폰트 로드를 요구한다. 실제로 안내 문구를 고치려다 실패해 노드를 삭제하는 것으로 우회했다.

> **해소됨 (2026-07-27):** **폰트를 Figma에 업로드**하니 MCP 컨텍스트에서도 보인다. 목록이 8,934 → 8,943개로 늘었고(Pretendard 9종), `loadFontAsync` 로 Regular·Medium·Bold 전부 로드된다. 라운드 2·3에서 Pretendard 텍스트를 자유롭게 만들고 수정할 수 있다.
>
> **핵심 교훈: 로컬 설치와 Figma 업로드는 다른 일이다.** `~/Library/Fonts` 에 2023년부터 있었고 macOS도 정상 인식했지만 MCP에는 보이지 않았다. Figma 파일에서 쓰는 것과 **에이전트가 스크립트로 쓰는 것**은 별개의 폰트 접근 경로이며, 후자를 열려면 [Figma에 폰트를 업로드](https://help.figma.com/hc/articles/360039956894-Add-a-font-to-Figma)해야 한다.
>
> `Pretendard Variable` 은 여전히 없다(static 9종만 업로드됨). 코드는 `"Pretendard Variable", Pretendard, …` 순으로 지정하므로 실사용에 문제는 없다.

**라운드 2에 남는 전제**: 새 폰트를 도입할 때는 로컬 설치가 아니라 **Figma 업로드 여부**를 먼저 확인할 것. 목록에 없으면 그 폰트를 쓰는 노드는 생성 후 수정이 불가능해진다.
...
### ~~미결~~ 해결됨: `baseline.json`의 역할 (2026-07-27)

**결정: 스냅샷 방식을 폐기하고 소비처 대조로 교체했다.** `baseline.json` 과 `update-baseline` 스크립트를 삭제했으며, 세 번째 선택지였던 "역할 전환"은 택하지 않았다.

이유는 §8.1·8.2에 있다 — 값 대조는 이사 검증에는 맞았지만 상시 방어에는 맞지 않는다. 값 변경 감시는 커밋된 `dist/tokens.css` 의 git diff 가 더 정확히 하고, 이름 소실은 소비처 대조가 더 넓게 잡는다. `INTENTIONAL_VALUE_CHANGES`(이사 전용 예외 목록)도 함께 사라졌다.

**라운드 2 이후 값이 바뀔 때 할 일:** `tokens/*.json` 을 고치고 `npm run build` 를 돌린 뒤, **`dist/tokens.css` 의 diff 를 리뷰에서 확인**한다. 별도의 승인 절차나 스냅샷 갱신은 없다.
