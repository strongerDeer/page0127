...
- `/dashboard` 건(lastSeen 2026-07-23, 1회)이 기준 시각 2026-07-25에서 🟡 지켜보세요로 분류되는가
- 같은 이슈를 기준 시각 2026-08-05로 평가하면 🟡 잠잠해짐으로 바뀌는가 (7일 규칙)
- 노이즈 패턴 5종이 각각 🚫로 분류되는가
- `Text content does not match`가 🚫로 **분류되지 않는가** (역방향 검증)
- 한글 메시지(`도서 검색 실패: ...`)가 ⚪로, 영어 크래시가 ⚪가 **아닌 것**으로 분류되는가
- `lastSeen`이 12시간 전이면 🔴, `firstSeen~lastSeen` 간격이 3일 이상이면 🔴

판정이 현재 시각에 의존하므로(4·5번 규칙) `triage(issue, now)`로 기준 시각을 주입받는다. 테스트는 고정 시각을 넘긴다.

### 1번 규칙은 triage가 아니라 조회 단계에서 처리한다

이슈 객체에는 environment 필드가 없다. 대신 조회 URL에 `environment=vercel-production`을 넣어 **API가 걸러서 주도록** 한다. 따라서 `triage()`는 2~6번 규칙만 담당한다. 실측으로 이 파라미터가 정확히 3건만 반환함을 확인했다.
