...
> **함정 2 — 분포 차트는 단순 치환이 아니다.** 분포는 **rating 값으로 그룹핑**해
> "1점 n권 … 5점 n권, 인생책 n권"을 그린다. 지금은 인생책이 `rating = 10`이라 자연히
> 별도 항목으로 갈렸다. 백필 후에는 **인생책도 `rating = 5`가 되어 5점 항목에 합쳐진다**
> — 치환만 하면 차트에서 인생책 항목이 사라지고 5점 막대가 갑자기 커진다. 에러도 안 나고
> 합계도 맞아서 눈치채기 어렵다. 그룹핑 키를 `rating` 단독에서 `(rating, is_life_book)`
> 조합으로 바꿔야 한다.
>
> 고칠 자리가 표시 컴포넌트가 아니라 **집계 함수 두 곳**이라는 점이 중요하다. 차트는
> 데이터를 prop 으로 받는다 — `libraryPeriod.ts`의 `calculateRatingReading`(연도별)과
> `getOverallStats.ts`의 `calculateRatingDistribution`(전체)이 각자 따로 구현돼 있고,
> 버킷 정의 상수(`VALID_RATINGS`·`RATING_COLORS`)와 두 타입(`RatingReadingData`·
> `RatingDistribution`)도 함께 바뀐다.

> **함정 3 — "인생책만 보기" 필터가 5점 책을 함께 끌고 온다.** 분포 항목을 클릭하면 그
> 평점의 책만 걸러 보여 주는데, 체인이 `rating` 숫자 하나로만 되어 있다:
>
> ```
> RatingDoughnutChart(onRatingClick(item.rating)) → DashboardCharts → LibraryView
>   → useLibraryFilters(selectedRating: number | null) → DashboardBookList
>       if (book.rating !== selectedRating) return false;
> ```
>
> 백필 후 인생책을 클릭하면 `selectedRating = 5`가 되어 평범한 5점 책이 함께 걸리고,
> 필터 칩도 "5점"이라고 표시된다. 필터 상태에 인생책을 표현할 값(`number | 'life'`)이
> 필요하며 5개 파일이 바뀐다.

**함정 2와 3이 이 트랙에서 로직이 실제로 바뀌는 유일한 두 곳이다.** 나머지는 기계적
치환이므로, 구현·리뷰의 주의를 이 둘에 집중한다.
...
| 7 | 분포 그룹핑 수정 + 테스트 (함정 2) | 중간 |
| 8 | 필터 체인 — 인생책 구분 (함정 3) | 중간 |
| 9 | 쓰기 경로 (등록 폼 매핑) | 작음 |
...
7·8번을 6번에서 떼어낸 이유: 이 둘만 **기계적 치환이 아니라 판단이 들어가는 변경**이다
(함정 2·3). 큰 치환 커밋에 섞이면 안 읽힌 채 통과할 위험이 가장 큰 조각이므로 독립
커밋으로 분리해 눈에 띄게 둔다.

> 실제 커밋 번호와 단계별 코드는 `docs/superpowers/plans/2026-07-28-rating-life-book-split.md`
> 를 따른다. 이 표는 규모 감각을 위한 것이다.
