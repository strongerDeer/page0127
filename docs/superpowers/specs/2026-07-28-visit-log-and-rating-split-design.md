...
`src/shared/lib/visit/kstDate.ts`

```ts
// Intl로 계산한다 — date-fns-tz 같은 새 의존성을 들이지 않는다.
// 'en-CA' 로케일이 YYYY-MM-DD 형식을 준다.
export const toKstDate = (at: Date): string => ...
```

서버와 클라이언트가 **같은 함수**를 쓴다. 클라이언트는 "오늘 이미 보냈는가" 판정에,
서버는 실제 저장값 결정에 쓴다. 브라우저 타임존이 무엇이든 Asia/Seoul로 강제하므로
해외 접속에서도 두 값이 어긋나지 않는다.
...
- `kstDate.test.ts`
  - UTC 2026-07-28 14:59Z → KST `2026-07-28` (23:59)
  - UTC 2026-07-28 15:00Z → KST `2026-07-29` (00:00) ← **날짜 경계**
  - 연말 경계: UTC 2026-12-31 15:00Z → `2027-01-01`
- `shouldReport.test.ts` (localStorage 값 + 오늘 날짜 → boolean)
  - 저장값 없음 → true
  - 저장값이 어제 → true
  - 저장값이 오늘 → false
