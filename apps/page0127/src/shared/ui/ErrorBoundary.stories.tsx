 ...
 * ## 접근성 — 조용히 바뀌면 안 된다
 *
 * 화면이 대체 UI 로 갈아끼워지는 것은 **눈으로 보는 사람에게만** 명백하다.
 * 스크린리더 사용자는 방금 실패했다는 사실 자체를 모른 채 이전 내용을 계속 읽고 있다고
 * 믿는다. 그래서 기본 `ErrorFallback` 은 `role='alert'` 을 갖는다 — 에러는 하던 일을
 * 멈추고 알려야 하는 소식이라 `polite` 가 아니라 `alert`(assertive)다.
 *
 * **직접 `fallback` 을 넘길 때도 같은 책임이 따라온다.** 그 노드에 `role='alert'` 을
 * 주지 않으면 그 자리만 조용히 바뀐다. 다만 `fallback={null}` 처럼 "없어도 되는 곁다리"는
 * 알릴 것이 없으므로 예외다.
 *
 * 제목은 실제 `<h2>` 로 둔다. `CardTitle` 은 `<div>` 라 그것만으로는 제목 목록에 안 잡힌다.
 *
 * ## 클래스 컴포넌트인 이유
 *
