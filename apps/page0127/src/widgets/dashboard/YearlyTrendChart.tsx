...
export const YearlyTrendChart = ({ data }: Props) => {
  ...
  // 최고 기록 연도 찾기
  const peakYear = data.reduce((max, item) => (item.count > max.count ? item : max));
  ...
  const growthRate =
    previousYear && previousYear.count > 0
      ? Math.round(((currentYear.count - previousYear.count) / previousYear.count) * 100)
      : 0;
    ...
    <div className="flex flex-col gap-2">
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data}>
          ...
          <Bar
            ...
            shape={(props: any) => {
              const isPeak = props.payload?.year === peakYear.year;
              return (
                <rect
                  x={props.x}
                  y={props.y}
      ...
      <div className="text-center text-sm text-gray-600">
        {currentYear.year === peakYear.year ? (
          <p>
            <span className="font-semibold text-emerald-600">{currentYear.year}년</span>이 역대 최고 기록이에요!
            🎉
          </p>
        ) : (
          <p>
            <span className="font-semibold text-emerald-600">{peakYear.year}년</span>이 최고 기록이에요. (
            {peakYear.count}권)
          </p>
        )}
