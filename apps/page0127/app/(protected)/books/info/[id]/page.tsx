...
async function getBookStats(isbn: string) {
  ...
  return { completedCount: completedCount || 0, avgRating, ratingCount: ratings?.length || 0 };
}
...
export default async function GlobalBookDetailPage({ params }: PageProps) {
  ...
  if (user) {
    const { count } = await supabase.from('books').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('isbn', book.isbn);
    isInLibrary = !!count;
  }
        ...
        <div className='grid gap-6 md:grid-cols-[240px_1fr]'>
          ...
          <div className='space-y-6'>
            <div>
              ...
              <p className='text-gray-500'>{book.publisher} · {book.pub_date}</p>
            </div>
            ...
            <div className='grid grid-cols-2 gap-4'>
               ...
               <Card>
                 <CardContent className='flex flex-col items-center justify-center p-6'>
                    ...
                    <p className='text-2xl font-bold'>{stats.avgRating} <span className='text-sm text-gray-400'>({stats.ratingCount})</span></p>
                 </CardContent>
               </Card>
            </div>
